...
namespace PayPal.v1.Subscriptions
{
    ...
    public class SubscriptionCancelRequest : HttpRequest
    {
        public SubscriptionCancelRequest(string SubscriptionId) : base("/v1/billing/subscriptions/{subscription_id}/cancel?", HttpMethod.Post, typeof(void))
        {
            try
            {
                this.Path = this.Path.Replace("{subscription_id}", Uri.EscapeDataString(Convert.ToString(SubscriptionId)));
            }
            catch (IOException) { }
        ...
        public SubscriptionCancelRequest RequestBody(string reason)
        {
            this.Body = new CancelSubscriptionBody() { Reason = reason };
            return this;
        }
    }
